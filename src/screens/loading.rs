...
impl<S: States> Plugin for LoadscreenPlugin<S> {
  fn build(&self, app: &mut App) {
    app.add_systems(OnEnter(self.state.clone()), setup_loadscreen);
    app.add_systems(
      Last,
      update_loading_pct
...
fn setup_loadscreen(mut commands: Commands) {
  ...
  let container = commands
    .spawn((
      StateDespawnMarker,
      NodeBundle {
        background_color: BackgroundColor(colors::PRIMARY_100),
        style: Style {
          width: Val::Auto,
          height: Val::Auto,
  ...
  let inner = commands
    .spawn((
      ...
      NodeBundle {
        background_color: BackgroundColor(colors::PRIMARY_200),
        style: Style {
